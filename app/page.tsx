...
const T = {
  en: {
    ...
    bn_feed: 'Feed',         bn_excl: 'Exclusive',
    bn_msg: 'Chat',          bn_acc: 'Profile',
    b_excl_title: 'My exclusive',
    ei_p: 'Purchased',       ei_ip: 'In progress',
    ei_dl1: 'Deadline: Jan 25',
    b_msgs: 'Messages',
    b_coins: 'Coins',        b_purch: 'Purchases',  b_spent: 'Spent',
    b_topup: 'Top up balance',
    b_ranks: 'Ranks & privileges',
    b_edit: 'Edit profile',
    b_pay: 'Payments',
    b_sec: 'Security',
  },
  ru: {
    ...
    bn_feed: 'Лента',        bn_excl: 'Эксклюзив',
    bn_msg: 'Чат',           bn_acc: 'Профиль',
    b_excl_title: 'Мой эксклюзив',
    ei_p: 'Куплено',         ei_ip: 'В работе',
    ei_dl1: 'Дедлайн: 25 янв',
    b_msgs: 'Сообщения',
    b_coins: 'Монеты',       b_purch: 'Покупки',    b_spent: 'Потрачено',
    b_topup: 'Пополнить баланс',
    b_ranks: 'Ранги и привилегии',
...
function GirlCard({ girl, onClick }: { girl: typeof newGirls[0]; onClick: () => void }) {
  return (
    <div onClick={onClick} style={{ background: '#141414', borderRadius: 12, overflow: 'hidden', border: '.5px solid #1e1e1e', cursor: 'pointer', flexShrink: 0, width: 120 }}>
      <div style={{ width: '100%', height: 90, position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', background: girl.bg }}>
        {girl.badge && (
          <div style={{ position: 'absolute', top: 6, left: 6, background: '#c9a96e', color: '#0a0a0a', fontSize: 8, fontWeight: 500, padding: '1px 6px', borderRadius: 20 }}>
            {girl.badge}
          </div>
        )}
        {girl.online && (
        ...
        <div style={{ width: 44, height: 44, borderRadius: '50%', border: '2px solid rgba(201,169,110,.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, color: '#555', textAlign: 'center' }}>
          Photo
        </div>
      </div>
...
// ─── Feed tab ─────────────────────────────────────────────────────────────────
function FeedTab({ lang, t, onOpenProfile }: { lang: Lang; t: typeof T['en']; onOpenProfile: () => void }) {
  return (
    <div style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden' }}>
      {/* New girls */}
      <div style={{ paddingTop: 14 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10, padding: '0 12px' }}>
          <h2 style={{ fontSize: 12, fontWeight: 500, color: '#888', letterSpacing: 1, textTransform: 'uppercase', margin: 0 }}>{t.b_new}</h2>
          <span style={{ fontSize: 11, color: '#c9a96e', cursor: 'pointer' }}>{t.b_see_new}</span>
        </div>
      </div>
      ...
      <div style={{ height: .5, background: '#1e1e1e', margin: '4px 12px' }} />

      {/* All girls */}
      <div style={{ paddingTop: 4 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10, padding: '0 12px' }}>
          <h2 style={{ fontSize: 12, fontWeight: 500, color: '#888', letterSpacing: 1, textTransform: 'uppercase', margin: 0 }}>{t.b_all}</h2>
          <span style={{ fontSize: 11, color: '#c9a96e', cursor: 'pointer' }}>{t.b_see_all}</span>
        </div>
      </div>
      <div style={{ display: 'flex', gap: 8, overflowX: 'auto', padding: '0 12px 12px' }}>
      ...
      <div style={{ borderRadius: 20, padding: 14, display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer', margin: '8px 12px', background: 'rgba(201,169,110,.1)', border: '1px solid rgba(201,169,110,.2)' }}>
        <div style={{ width: 42, height: 42, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, flexShrink: 0, background: 'rgba(201,169,110,.13)', border: '1px solid rgba(201,169,110,.27)', color: '#c9a96e' }}>🛍</div>
        <div>
          <div style={{ fontSize: 14, fontWeight: 500, color: '#c9a96e' }}>{t.b_mkt}</div>
          <div style={{ fontSize: 11, color: '#888', marginTop: 2 }}>{t.b_mkt_sub}</div>
        </div>
        <div style={{ marginLeft: 'auto', color: '#c9a96e', fontSize: 14 }}>›</div>
      </div>
      ...
      <div style={{ borderRadius: 20, padding: 14, display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer', margin: '8px 12px', background: '#141414', border: '1px solid rgba(201,169,110,.15)' }}>
        <div style={{ width: 42, height: 42, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, flexShrink: 0, background: 'rgba(201,169,110,.1)', border: '1px solid rgba(201,169,110,.2)', color: '#c9a96e' }}>🃏</div>
        <div>
        ...
        <div style={{ marginLeft: 'auto', color: '#555', fontSize: 14 }}>›</div>
      </div>
      ...
      <div style={{ paddingTop: 4 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10, padding: '0 12px' }}>
          <h2 style={{ fontSize: 12, fontWeight: 500, color: '#888', letterSpacing: 1, textTransform: 'uppercase', margin: 0 }}>{t.b_top}</h2>
          <span style={{ fontSize: 11, color: '#c9a96e', cursor: 'pointer' }}>Top 10</span>
        </div>
      </div>
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', padding: '12px 0' }}>
      ...
      <div style={{ marginBottom: 6 }}>
        ...
        {[
          { icon: '📷', title: 'Alina — Pack #1', sub: t.ei_p, price: '90c', deadline: null },
      ...
      {[
        ...
        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '9px 12px', background: '#141414', borderRadius: 12, margin: '0 12px 6px', cursor: 'pointer' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4 }}>
            ...
            {m.unread && <div style={{ width: 7, height: 7, background: '#c9a96e', borderRadius: '50%', flexShrink: 0 }} />}
          </div>
        </div>
      ))}
    </div>
...
function AccTab({ t }: { t: typeof T['en'] }) {
  return (
    <div style={{ flex: 1, overflowY: 'auto', paddingTop: 12 }}>
      ...
      <div style={{ background: '#141414', borderRadius: 14, border: '.5px solid #1e1e1e', padding: 14, margin: '0 12px 10px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
          <div style={{ position: 'relative' }}>
            <div style={{ width: 50, height: 50, borderRadius: '50%', background: '#1e1e1e', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, color: '#c9a96e', border: '2px solid #aaa' }}>👤</div>
          ...
          <div style={{ marginLeft: 'auto', color: '#c9a96e', fontSize: 17, cursor: 'pointer' }}>✎</div>
        </div>
      ...
      {/* Menu */}
      <div style={{ background: '#141414', borderRadius: 14, border: '.5px solid #1e1e1e', padding: '0 14px', margin: '0 12px 10px', display: 'flex', flexDirection: 'column', gap: 0 }}>
        {[
          { icon: '🪙', label: t.b_topup, right: '220c ↗', color: '#c9a96e' },
          { icon: '🏆', label: t.b_ranks, right: '›', color: '#c9a96e' },
          { icon: '👤', label: t.b_edit, right: '›', color: '#c9a96e' },
          { icon: '💳', label: t.b_pay,  right: '›', color: '#c9a96e' },
          { icon: '🔒', label: t.b_sec,  right: '›', color: '#c9a96e' },
        ].map((item, i, arr) => (
          <div key={i}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, color: '#aaa', fontSize: 13, cursor: 'pointer', padding: '13px 0' }}>
              <span style={{ fontSize: 17, color: '#c9a96e' }}>{item.icon}</span>
              <span>{item.label}</span>
              <span style={{ marginLeft: 'auto', fontSize: 11, color: '#c9a96e' }}>{item.right}</span>
            </div>
            {i < arr.length - 1 && <div style={{ height: .5, background: '#1e1e1e' }} />}
          </div>
        ))}
      </div>
    </div>
...
// ─── Girl Profile screen ──────────────────────────────────────────────────────
function GirlProfileScreen({ lang, t, onBack }: { lang: Lang; t: typeof T['en']; onBack: () => void }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', background: '#0a0a0a' }}>
      {/* Top bar */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px 10px', borderBottom: '.5px solid #1e1e1e', flexShrink: 0, background: '#0a0a0a' }}>
        <div onClick={onBack} style={{ color: '#c9a96e', fontSize: 20, cursor: 'pointer' }}>←</div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden' }}>
        ...
        <div style={{ padding: '36px 16px 12px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
            <div style={{ fontSize: 18, fontWeight: 500, color: '#f0f0f0' }}>Sofia</div>
            <div style={{ background: 'rgba(201,169,110,.1)', border: '.5px solid rgba(201,169,110,.27)', borderRadius: 20, padding: '2px 7px', fontSize: 9, color: '#c9a96e' }}>✓ Verified</div>
          </div>
        ...
        {/* Stats */}
        <div style={{ display: 'flex', borderTop: '.5px solid #1e1e1e', borderBottom: '.5px solid #1e1e1e', marginBottom: 12 }}>
          {[['1.2K','Subscribers'],['48','Packs'],['342','Orders']].map(([v,l],i,arr) => (
        ...
        <div style={{ display: 'flex', gap: 8, padding: '0 16px', marginBottom: 12 }}>
          ...
          <div style={{ width: 42, height: 42, background: '#141414', border: '.5px solid #2a2a2a', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#c9a96e', cursor: 'pointer', flexShrink: 0, fontSize: 17 }}>💬</div>
        </div>
        {/* Tabs */}
        ...
        {activeTab === 'packs' && (
          <div style={{ paddingTop: 12, paddingBottom: 20 }}>
            ...
            <div style={{ background: '#141414', borderRadius: 12, border: '.5px solid rgba(76,175,80,.2)', margin: '0 16px 8px', overflow: 'hidden' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 12px' }}>
                <div>
                  <div style={{ fontSize: 13, fontWeight: 500, color: '#f0f0f0' }}>Preview pack</div>
                  <div style={{ fontSize: 10, color: '#555', marginTop: 2 }}>5 photos • Free</div>
                </div>
            ...
            {[
              { name: 'January pack', meta: '24 photos', price: '500 🪙', locked: 3 },
              { name: 'Video pack',   meta: '3 videos',  price: '1,200 🪙', locked: 2 },
              { name: 'Exclusive set',meta: '15 photos + 1 video', price: '2,000 🪙', locked: 3 },
            ].map(pack => (
              <div key={pack.name} style={{ background: '#141414', borderRadius: 12, border: '.5px solid #1e1e1e', margin: '0 16px 8px', overflow: 'hidden' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 12px' }}>
                  <div>
                ...
                <div style={{ display: 'flex', gap: 4, padding: '0 12px 8px' }}>
                  {[...Array(3)].map((_,i) => (
                    <div key={i} style={{ width: 46, height: 46, background: '#1e1e1e', borderRadius: 7, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: i < (3 - pack.locked) ? 13 : 11 }}>
                      {i < (3 - pack.locked) ? '📷' : '🔒'}
                    </div>
                  ))}
                </div>
          ...
          <div style={{ padding: '14px 16px 20px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 4 }}>
              {[['#1a1510',''],['#12101a',''],['#0f1a14',''],['#1a100f','🔒'],['#10121a','🔒'],['#1a1218','🔒']].map(([bg,lock],i) => (
                <div key={i} style={{ aspectRatio: '1', background: bg, borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: lock ? 20 : 10, color: '#333', position: 'relative', overflow: 'hidden' }}>
...
export default function Home() {
    ...
    return (
      <div style={{ background: '#0a0a0a', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem', fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif' }}>
        <div style={{ maxWidth: 360, width: '100%', textAlign: 'center' }}>
          ...
          <button onClick={() => setEntered(true)} style={{ width: '100%', background: '#c9a96e', color: '#0a0a0a', border: 'none', borderRadius: 50, padding: 14, fontSize: 15, fontWeight: 500, cursor: 'pointer', marginBottom: 10 }}>{t.age_yes}</button>
  ...
  // ── BUYER APP ───────────────────────────────────────────────────────────────
  return (
    <div style={{ background: '#0a0a0a', minHeight: '100vh', display: 'flex', justifyContent: 'center', fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif' }}>
      <div style={{ width: '100%', maxWidth: 480, display: 'flex', flexDirection: 'column', minHeight: '100vh', position: 'relative' }}>

        {/* Girl profile overlay */}
        {showProfile && (
          <GirlProfileScreen lang={lang} t={t} onBack={() => setShowProfile(false)} />
        )}

        {/* TOP BAR */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px 10px', borderBottom: '.5px solid #1e1e1e', flexShrink: 0, background: 'rgba(10,10,10,.95)', backdropFilter: 'blur(10px)', position: 'sticky', top: 0, zIndex: 50 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            {/* Icons */}
            <div style={{ display: 'flex', gap: 12, color: '#666', fontSize: 18 }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          {buyerTab === 'feed' && <FeedTab lang={lang} t={t} onOpenProfile={() => setShowProfile(true)} />}
        ...
        {/* BOTTOM NAV */}
        <div style={{ background: '#0f0f0f', borderTop: '.5px solid #1e1e1e', display: 'flex', padding: '8px 0 10px', flexShrink: 0, position: 'sticky', bottom: 0 }}>
          {([
            ['feed', '🔥', t.bn_feed],
            ['excl', '⭐', t.bn_excl],
            ['msg',  '💬', t.bn_msg],
            ['acc',  '👤', t.bn_acc],
          ] as [BuyerTab, string, string][]).map(([tab, icon, label]) => (
            <div key={tab} onClick={() => setBuyerTab(tab)} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, cursor: 'pointer', color: buyerTab===tab?'#c9a96e':'#444' }}>
